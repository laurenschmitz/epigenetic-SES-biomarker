...
/*mediation models and table of results*/
%macro mediate(out, dist, aorc);
%if &aorc=a %then %do;
	proc causalmed data=phenos4 descending;
		class ragender race smoke;
		model &out = zSESindex_mclock zSESindex / dist=&dist;
		mediator zSESindex_mclock = zSESindex;
		covar page race ragender smoke;
		ods output EffectSummary=effects;
	run;
%end;
...
%else %do;
	proc causalmed data=phenos4 descending;
		class ragender race smoke;
		model &out = zcSES_mclock zcSES / dist=&dist;
		mediator zcSES_mclock = zcSES;
		covar page race ragender smoke;
		ods output EffectSummary=effects;
	run;
%end;

data effects_&out._&aorc;
	set effects;
	if effect in ('Total Effect', 'Natural Direct Effect (NDE)', 'Natural Indirect Effect (NIE)', 'Percentage Mediated', 'Odds Ratio Total Effect', 'Odds Ratio Natural Direct Effect (NDE)', 'Odds Ratio Natural Indirect Effect (NIE)');
	effectci = cat(strip(put(estimate,5.2)), ' (', strip(put(LowerWaldCL,5.2)), ', ', strip(put(UpperWaldCL,5.2)), ')');
run;

proc transpose data=effects_&out._&aorc out=teffects_&out._&aorc(drop=_NAME_);
	var effectci;
run;

data teffects_&out._&aorc;
	set teffects_&out._&aorc;
	outcome = "&out";
	Total_Effect=COL1;
	NDE = COL2;
	NIE = COL3;
	perc_mediated = COL4;
	keep outcome--perc_mediated;
run;
%mend;


ods pdf;
%mediate(r14conde, poisson, a)
%mediate(zMSI18, normal, a)
%mediate(r14shlt, normal, a)
%mediate(dead18, binomial, a)
%mediate(langaweir18_dementia, binomial, a)
ods pdf close;

data sesindex_med;
	set teffects_r14conde_a teffects_zMSI18_a teffects_r14shlt_a teffects_dead18_a teffects_langaweir18_dementia_a;
run;

ods rtf;
proc print data=sesindex_med noobs;
run;
ods rtf close;


ods pdf;
%mediate(r14conde, poisson, c)
%mediate(zMSI18, normal, c)
%mediate(r14shlt, normal, c)
%mediate(dead18, binomial, c)
%mediate(langaweir18_dementia, binomial, c)
ods pdf close;

data cses_med;
	set teffects_r14conde_c teffects_zMSI18_c teffects_r14shlt_c teffects_dead18_c teffects_langaweir18_dementia_c;
run;

ods rtf;
proc print data=cses_med noobs;
run;
ods rtf close;
