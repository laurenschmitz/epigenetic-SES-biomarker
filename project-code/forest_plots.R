...
hrs.asesw[1]=c('HRS aSES-BIO', 'HRS cSES-BIO', 'HRS aSES-BIO450', 'HRS cSES-BIO450')
...
mesa.sesw[1]=c('MESA aSES-BIO450', 'MESA cSES-BIO450')
...
#combined plot
combo.ases = read.csv('S:/MESA/Methylation/Lauren_DNAmAge/results/HRS_Enet/HRS_MESA_SESindex_mclock_models_table_raw.csv')

combo.asesw = reshape(combo.ases, direction='wide', idvar='SES', timevar='Model')

combo.asesw$a <- paste(rep(" ", 26), collapse = " ")
combo.asesw$` ` <- paste(rep(" ", 2), collapse = " ")
combo.asesw$b <- paste(rep(" ", 26), collapse = " ")
combo.asesw$`  ` <- paste(rep(" ", 2), collapse = " ")
combo.asesw$c <- paste(rep(" ", 26), collapse = " ")
combo.asesw$`   ` <- paste(rep(" ", 2), collapse = " ")
combo.asesw$d <- paste(rep(" ", 26), collapse = " ")
combo.asesw$`    ` <- paste(rep(" ", 2), collapse = " ")
combo.asesw$e <- paste(rep(" ", 26), collapse = " ")


colnames(combo.asesw)[c(1,32,34,36,38,40)] = c('' ,'Number of\nchronic conditions', 'Cardiometabolic\nconditions index', 'Self-reported\nhealth status', 'Mortality', 'Dementia')
combo.asesw[1]=c('HRS aSES-BIO', 'HRS aSES-BIO450', 'MESA aSES-BIO450', '', 'HRS cSES-BIO', 'HRS cSES-BIO450', 'MESA cSES-BIO450')


tm = forest_theme(base_size = 16, colhead=list(fg_params=list(hjust=c(0,0.5,0.5,0.5,0.5,0.5,0.5), x=c(0,0.5,0.5,0.5,0.5,0.5,0.5))), core=list(bg_params=list(fill = c('white', 'white'))), xaxis_gp=gpar(fontsize=12), xlab_gp = gpar(fontsize = 12), xlab_adjust='center', legend_position = 'none')

p = forest(combo.asesw[,c(1,32:36)], 
           est=list(combo.asesw$estimate.r14conde.1,combo.asesw$estimate.zMSI18.1,combo.asesw$estimate.R14SHLT.1,combo.asesw$estimate.r14conde.2,combo.asesw$estimate.zMSI18.2,combo.asesw$estimate.R14SHLT.2), 
           lower=list(combo.asesw$lowercl.r14conde.1,combo.asesw$lowercl.zMSI18.1,combo.asesw$lowercl.R14SHLT.1,combo.asesw$lowercl.r14conde.2,combo.asesw$lowercl.zMSI18.2,combo.asesw$lowercl.R14SHLT.2), 
           upper=list(combo.asesw$uppercl.r14conde.1,combo.asesw$uppercl.zMSI18.1,combo.asesw$uppercl.R14SHLT.1,combo.asesw$uppercl.r14conde.2,combo.asesw$uppercl.zMSI18.2,combo.asesw$uppercl.R14SHLT.2), 
           ci_column = c(2,4,6), 
           ref_line = c(1,0,0),
           xlim=list(c(0.98,1.18),c(-.07,.22),c(-.07,.24)),
           ticks_at = list(c(1.0,1.04,1.08,1.12,1.16),c(-0.05,0.0,0.05,0.1,0.15,0.2),c(-0.05,0.0,0.05,0.1,0.15,0.2)),
           xlab = c('IRR','Beta','Beta'),
           nudge_y = .35,
           theme = tm)
...
tm2 = forest_theme(base_size = 16, colhead=list(fg_params=list(hjust=c(0,0.5,0.5,0.5,0.5), x=c(0,0.5,0.5,0.5,0.5))), core=list(bg_params=list(fill = c('white', 'white'))), xaxis_gp=gpar(fontsize=12), xlab_gp = gpar(fontsize = 12), xlab_adjust='center', legend_name = '', legend_value = c('Model 1  ','Model 2'), legend_position = 'bottom')

p2 = forest(combo.asesw[,c(1,38:40)], 
            est=list(combo.asesw$estimate.dead18.1,combo.asesw$estimate.langawei.1,combo.asesw$estimate.dead18.2,combo.asesw$estimate.langawei.2), 
            lower=list(combo.asesw$lowercl.dead18.1,combo.asesw$lowercl.langawei.1,combo.asesw$lowercl.dead18.2,combo.asesw$lowercl.langawei.2), 
            upper=list(combo.asesw$uppercl.dead18.1,combo.asesw$uppercl.langawei.1,combo.asesw$uppercl.dead18.2,combo.asesw$uppercl.langawei.2), 
            ci_column = c(2,4), 
            ref_line = c(1,1),
            xlim=list(c(0.69,2.1),c(0.64,1.7)),
            ticks_at = list(c(0.75,1.0,1.25,1.5,1.75,2.0),c(0.8,1.0,1.2,1.4,1.6)),
            xlab = c('OR','OR'),
            nudge_y = .35,
            theme = tm2)
...
png(filename = 'S:/MESA/Methylation/Lauren_DNAmAge/results/HRS_Enet/HRS_MESA_SESindex_mclock_models_forest_plot.png' , width = 900, height = 500,bg = "white",pointsize=24)
grid.arrange(p, p2)
dev.off()
